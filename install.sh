...
# 2. Copy example config (don't overwrite existing)
if [ ! -f "$RUNTIME_DIR/config.json" ]; then
  cp "$SKILL_DIR/config.example.json" "$RUNTIME_DIR/config.json"
  echo "[ok] Created config: $RUNTIME_DIR/config.json"
  echo "     Edit this file to configure your accounts."
else
  echo "[skip] Config already exists: $RUNTIME_DIR/config.json"
fi
...
if command -v python3 &> /dev/null; then
  if [ ! -d "$SKILL_DIR/.venv" ]; then
    ...
    echo "[ok] Python venv created: $SKILL_DIR/.venv"
  else
...
# 5. Install as Claude Code skill (optional)
echo ""
...
if [ -d "$SKILLS_DIR" ]; then
  ...
  echo "       ln -s $SKILL_DIR $SKILLS_DIR/vidpilot"
fi
...
echo ""
echo "Next steps:"
echo "  1. Edit $RUNTIME_DIR/config.json to configure your accounts"
echo "  2. Add character images to engine/public/"
echo "  3. Use 'ACCOUNT=yourname' with the TTS and render scripts"
echo ""
echo "Quick test:"
echo "  cd $SKILL_DIR/engine && npx remotion studio src/index.ts"
