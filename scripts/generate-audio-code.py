#!/usr/bin/env python3
"""
用 ChatTTS 生成代码演示旁白语音。

用法：
  source .venv/bin/activate
  ACCOUNT=laodong python scripts/generate-audio-code.py
"""
...
DATA_FILE, _voice_seeds = get_data_file_path(
    ACCOUNT_ID, "code",
    fallback_default="src/data/example/code.example.ts",
)
...
def parse_data_ts(filepath):
    with open(filepath, "r", encoding="utf-8") as f:
        content = f.read()
    match = re.search(r"export const \w+CodeSteps.*?=\s*\[(.*?)\];", content, re.DOTALL)
    if not match:
        raise ValueError(f"找不到 CodeSteps 数据 in {filepath}")
    steps = []
...
def main():
    output_path = os.path.join(PROJECT_DIR, "out/code-audio.wav")
    ...
    print(f"\n[info] 读取代码步骤: {DATA_FILE}")
    steps = parse_data_ts(DATA_FILE)
    print(f"[info] 共 {len(steps)} 步")

    torch.manual_seed(NARRATOR_SEED)
    narrator_spk = chat.sample_random_speaker()
    print(f"[info] 旁白 seed={NARRATOR_SEED}")

    clips_dir = os.path.join(PROJECT_DIR, "out/code-clips")
    ...
    for i, step in enumerate(steps):
        clip_path = os.path.join(clips_dir, f"code_{i:03d}.wav")
        ...
        text = normalize_text(step["narration"])
        ...
        print(f"  [{i+1}/{len(steps)}] {dur_s:.1f}s -> {frames}帧")
