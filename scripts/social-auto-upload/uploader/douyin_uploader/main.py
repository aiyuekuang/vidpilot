...
class DouYinVideo(object):
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        if await title_container.count():
            ...
            # version_2: 通过 placeholder 定位标题输入框
            title_v2 = page.locator('input[placeholder*="作品标题"]')
            if await title_v2.count():
                await title_v2.fill(self.title[:30])
            else:
        ...
        for index, tag in enumerate(self.tags, start=1):
            ...
            await page.keyboard.press("Escape")  # 关闭中文自动补全弹窗
            await page.keyboard.press("Space")   # 添加空格分隔
